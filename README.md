...
The headline workflow — roll a model through history, run all four tests, render
the verdict — is a single function. `run_backtest` returns a `BacktestReport`
that carries every result and knows how to print, plot, or save itself:
...
report.save("backtest.pdf")    # the print-ready A4 dashboard
report.save("report.pdf", sections="all")   # a selective, multi-page report

report.kupiec.p_value          # every number is a plain field, nothing hidden
report.traffic_light.zone
```

The saved report is **print-ready by default** — `run_backtest` generates the
title and a self-describing metrics footer (inputs · result · test verdicts)
from the run itself, so you pass data, not styling. Override any of `title` /
`subtitle` / `footer` if you want to.

That is all `examples/full_backtest.py` and `examples/single_instrument.py` do:
load the data, call `run_backtest`, print/save. The sections below show the
building blocks it composes, for when you want to drive them directly.

## Rolling a model through history

Backtesting starts by turning one series into a *series of VaR forecasts*. This
is the building block `run_backtest` rolls on, and it lives in `varlib.backtest`:
...
The `examples/` folder shows this in action, writing to `examples/output/`:

```bash
# give it data -> printed stats + a single labelled dashboard page
python examples/full_backtest.py

# each chart on its own, for any model:
#   --model historical|bootstrap|brownian|ou|jump|ewma   --confidence
...
`full_backtest.py` is deliberately a no-argument demo — the title and footer are
generated by the library, so it just loads the data and calls `run_backtest`. To
backtest a different model, change the one line that builds it (or use
`run_backtest` directly, as shown above). See **`examples/README.md`** for more.
...
varlib/
  ...
  report.py                run_backtest + BacktestReport: roll, test, print/save
...
examples/
  ...
  full_backtest.py         give it data -> run_backtest -> labelled report (no CLI)
