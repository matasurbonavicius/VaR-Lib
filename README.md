...
Every example is a no-argument script with no shared helpers — each loads the
data and calls the library directly, so you can read any one top to bottom. To
backtest a different model, change the one line that builds it (or call
...
examples/
  ...
  charts/                  one self-contained script per chart
